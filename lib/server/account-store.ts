...
function mapAccountStoreError(action: string, error: { message: string; code?: string | null }) {
  if (error.code === "23505") {
  ...
  if (error.code === "42P01") {
    return new AccountStoreError("The mail_accounts table is missing. Re-run the latest schema.", 500);
  }
...
export async function createAccount(
  ...
  try {
    ...
    throw mapAccountStoreError("create account", { code: pgError.code ?? null, message: pgError.message ?? "Database error" });
  }
}
...
export async function updateAccount(
  ...
  try {
    ...
    const pgError = error as { code?: string; message?: string };
    throw mapAccountStoreError("update account", { code: pgError.code ?? null, message: pgError.message ?? "Database error" });
  }
}
